...
题解：前缀和 + 哈希表。定义`pre[i]` 为 `[0..i]`里所有数的和。那么`pre[i]=pre[i−1]+nums[i]`。 `[j..i]` 这个子数组和为 k。

用k替换`nums[i]`可以推导出`pre[i−1] = pre[i] - k`。所以可以以前缀和`pre[i]`为k，然后pre[i]出现的次数为value存入hashMap 中，当出现了`pre[j] = pre[i] - k`时，（`pre[i] - k`作为key从hashMap中获取），子数组count++；最后count即为答案。
